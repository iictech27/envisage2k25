...
import { ReqSSRegistrationOrderBody } from "../api/bodies/registration_ss";
import { newReg } from "../api/handlers";
...
const RegisterWithUPI = ({ onClose }: RegisterProps) => {
  // Form state
  const [formData, setFormData] = useState<ReqSSRegistrationOrderBody>({
    fullName: "",
    email: "",
    phone: "",
    college: "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if(!paymentScreenshot) {
      alert("Please provide payment screenshot!");
      return;
    }

    // Process form submission
    formData.eventIDs = new Array<number>(selectedEvents.length);
    for(let i = 0; i < selectedEvents.length; i++) {
      formData.eventIDs[i] = selectedEvents[i].id;
    }

    const response = await newReg({
      ...formData,
    }, paymentScreenshot);
