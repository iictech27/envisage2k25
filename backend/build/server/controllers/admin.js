...
export const getRegistrations = async (_req, res, next) => {
    try {
...
export const verifyRegistration = async (req, res, next) => {
    ...
    try {
        if (!registrationID) {
            throw createHttpError(httpCodes["400"].code, httpCodes["400"].message + ": 'regID' parameter is missing!");
        }
        const reg = await SSRegistrationModel.findById(new mongoose.Types.ObjectId(registrationID)).exec();
        if (!reg) {
            throw createHttpError(httpCodes["400"].code, httpCodes["400"].message + ": No registration with given id exists");
        }
