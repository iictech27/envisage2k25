...
import { ReqEmailVeriBody, ReqLoginBody, ReqSignupBody, ResEmailVeriBody, ResUserBody, ResUserRegEventsBody } from "../bodies/user.js";
import UnverifiedUserModel from "../../db/models/unverified_user.js";
import mailOptions from "../mails/verif_email.js";
import transport from "../services/nodemailer.ts";
...
export const signUp: RequestHandler<unknown, unknown, ReqSignupBody, unknown> = async (req, res, next) => {
    ...
    try {
        ...
        const unverifiedUserWithEmail = await UnverifiedUserModel.findOne({ email: email }).exec();
        if(unverifiedUserWithEmail) {
            ...
            throw createHttpError(httpCodes["409"].code, httpCodes["409"].message + ": Sign up request already exists. Check your inbox and verify email.");
        }
...
export const verifyEmail: RequestHandler<unknown, unknown, ReqEmailVeriBody, unknown> = async (req, res, next) => {
    ...
    try {
        ...
        if(!sessionToken) {
            throw createHttpError(httpCodes["400"].code, httpCodes["400"].message + ": Please try to signup or login atleast once before verification!");
        }

        const unverifiedUser = await UnverifiedUserModel.findById(sessionToken).exec();

        if(!unverifiedUser) {
            throw createHttpError(httpCodes["403"].code, httpCodes["403"].message + ": No signup request for user!");
        }
        ...
        // otp has expired
        if(difference <= 0) {
            throw createHttpError(httpCodes["403"].code, httpCodes["403"].message + ": Verification code has expired! Request another.");
        }

        const otpMatched = await bcrypt.compare(otp, unverifiedUser.hashedOtp);
        if(!otpMatched) {
            throw createHttpError(httpCodes["401"].code, httpCodes["401"].message + ": Invalid OTP!");
        }
...
export const resendVerifyEmail: RequestHandler = async (req, res, next) => {
    ...
    try {
        ...
        if(difference >= 0) {
            throw createHttpError(httpCodes["403"].code, httpCodes["403"].message + ": Please wait for atleast 1 minute before sending new request!");
        }
