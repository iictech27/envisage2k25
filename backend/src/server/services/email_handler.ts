...
export const sendMail = async (mailOpts: MailOptions): Promise<any> => {
  if (!mailOpts.to || !mailOpts.subject || !mailOpts.text) {
    ...
    throw createHttpError(
      httpCodes["502"].code,
      httpCodes["502"].message +
        ": Could not send email! Please try again later. Inconvenience regretted"
    );
  }
  ...
  try {
    ...
    logWarn(
      ...
      "sendMail @ services/email_handler.ts"
    );
    logErr(error, "sendMail @ services/email_handler.ts");
    logInfo(
      "Trying to send mail using gmail instead!",
      "sendMail @ services/email_handler.ts"
    );
  }
  ...
  try {
    ...
    logWarn(
      "Failed to send email using gmail!",
      "sendMail @ services/email_handler.ts"
    );
    logErr(error, "sendMail @ services/email_handler.ts");
